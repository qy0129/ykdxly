...
public final class NearbyFoodWorkflow {
    ...
    private final Map<String, List<com.example.ilink.feature.travel.AmapService.Place>> pendingLocations = new ConcurrentHashMap<>();
    ...
    public void handle(ILinkClient client, String userId, IntentResult route) throws Exception {
        ...
        searchAndReply(client, userId, currentLocation, "", "");
    }
    ...
    public void handleLocationSelection(ILinkClient client, String userId, String text) throws Exception {
        ...
        List<com.example.ilink.feature.travel.AmapService.Place> candidates = pendingLocations.get(userId);
        try {
    ...
    private void searchAndReply(ILinkClient client, String userId, String location,
        ...
        if (output != null && output.mapImage() != null) {
            client.sendImage(userId, output.mapImage(), "nearby-food-map.png", "附近餐饮地点地图");
        }
        if (output != null && !output.candidates().isEmpty()) {
            pendingLocations.put(userId, output.candidates());
        }
