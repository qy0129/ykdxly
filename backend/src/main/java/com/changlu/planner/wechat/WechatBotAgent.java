...
public final class WechatBotAgent implements AutoCloseable {
  ...
  private String configuredWebUrl() { String configured = System.getenv().getOrDefault("PLANNER_WEB_URL", "").trim(); return configured.isBlank() ? "http://" + lanAddress() + ":8081/" : configured; }
  private String lanAddress() {
    try { String fallback = ""; for (NetworkInterface network : Collections.list(NetworkInterface.getNetworkInterfaces())) { if (!network.isUp() || network.isLoopback()) continue; for (InetAddress address : Collections.list(network.getInetAddresses())) { if (address instanceof Inet4Address && !address.isLoopbackAddress()) { if (fallback.isBlank()) fallback = address.getHostAddress(); if (network.getName().toLowerCase().contains("wi") || network.getDisplayName().toLowerCase().contains("ethernet")) return address.getHostAddress(); } } } if (!fallback.isBlank()) return fallback; } catch (Exception ignored) { }
    return "127.0.0.1";
  }
