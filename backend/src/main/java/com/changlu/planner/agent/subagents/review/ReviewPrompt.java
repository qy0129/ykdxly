...
public final class ReviewPrompt {
  ...
  public static JsonArray messages(JsonObject facts) {
    ...
    messages.add(ModelClient.message("system", """
        ...
        summary、highlights、risks、nextActions 里的文字必须是纯自然中文正文，禁止出现任何代码相关的内容：
        不要用 ``` 代码块，不要用反引号包裹任何词，不要输出 JSON 片段或 "summary:"、"highlights:" 这类键名，
        不要出现编程语言名、变量名、HTML/XML 标签、正则表达式或命令行语法。引用计划/任务名时直接写中文原名即可。
        只输出 JSON（外壳是给系统解析的，字段值按上述要求写纯中文）：
